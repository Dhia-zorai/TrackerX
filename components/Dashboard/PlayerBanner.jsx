  ...
  if (loading) {
    return (
      <div className='glass rounded-xl px-4 py-3 flex flex-col items-center gap-1 animate-pulse'>
        <div className='w-5 h-5 bg-[var(--bg-elevated)] rounded' />
        ...
        <div className='relative shrink-0'>
          ...
          >
            ...
            <div className='absolute inset-0 opacity-25' style={{
              background: 'radial-gradient(circle at 30% 25%, rgba(255,255,255,0.5), transparent 65%)',
            }} />
          </div>
          {/* Level badge */}
          {level > 0 && (
            <div className='absolute -bottom-1 -right-1 w-6 h-6 rounded-full bg-[var(--bg-elevated)] border border-[var(--border-accent)] flex items-center justify-center'>
              <span className='text-[9px] font-bold text-[var(--accent)] tabular-nums leading-none'>{level}</span>
            </div>
          )}
        </div>
