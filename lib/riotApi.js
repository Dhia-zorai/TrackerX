...
async function riotFetch(url, retries = 3) {
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      if (res.status === 403) { const e = new Error("This endpoint requires a production Riot API key. Development keys only support account lookups."); e.status = 403; throw e; }
...
export async function getMatchList(puuid, region = "na", count = 20) {
  const host = REGION_HOSTS[region] || REGION_HOSTS.na;
  const data = await riotFetch(host + "/val/match/v1/matchlists/by-puuid/" + puuid);
  return { ...data, history: (data.history || []).slice(0, count) };
}
