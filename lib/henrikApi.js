...
export async function henrikGetMatchesByPuuid(puuid, region = "na", size = 10, mode = "") {
  ...
  for (const match of (rawList || [])) {
    ...
    statsRows.push(computeMatchStats(match, puuid));
  }
  ...
  try {
    if (matchRows.length > 0) {
      ...
      if (matchUpsertError) {
        // Silent — cache errors are non-critical for response
      }
    }
    ...
    if (statsRows.length > 0) {
      const { error: statsUpsertError } = await supabase
        ...
        .upsert(statsRows, { onConflict: 'puuid,match_id' });
      if (statsUpsertError) {
        // Silent — cache errors are non-critical for response
      }
    }
...
export async function henrikGetLifetimeMatches(name, tag, region = "na", page = 1, size = 10, mode = '') {
  ...
  // Cache full matches before returning so serverless doesn't drop writes.
