...
async function fetchAndCacheMatch(matchId, puuid) {
  ...
  // Check if we have this stat combo cached AND the match data
  const [statsRes, matchRes] = await Promise.all([
    supabase.from('player_match_stats').select('match_id').eq('puuid', puuid).eq('match_id', matchId).single(),
    supabase.from('matches').select('raw_json').eq('match_id', matchId).single()
  ]);

  if (statsRes.data && matchRes.data?.raw_json) {
    console.log(`[Cache] Match ${matchId} for puuid ${puuid} found in DB.`);
    return matchRes.data.raw_json;
  }
  ...
  const matchPromise = supabase.from('matches').upsert({
  ...
  const statsPromise = supabase.from('player_match_stats').upsert(stats, { onConflict: 'puuid,match_id' });

  const [upsertMatchRes, upsertStatsRes] = await Promise.all([matchPromise, statsPromise]);
  if (upsertMatchRes.error) console.error('[Supabase] matches upsert error:', upsertMatchRes.error.message);
  if (upsertStatsRes.error) console.error('[Supabase] player_match_stats upsert error:', upsertStatsRes.error.message);

  return data;
}

export async function henrikGetMatchesByPuuid(puuid, region = "na", size = 10, mode = "") {
  const r = HENRIK_REGIONS[region] || "na";
  let endpoint = `/valorant/v3/by-puuid/matches/${r}/${puuid}?size=${size}`;
  if (mode && mode !== 'all') endpoint += `&mode=${mode}`;
  
  let rawList;
  try {
    rawList = await henrikFetch(endpoint);
  } catch (err) {
    if (mode && (err.status === 404 || err.status === 400)) {
      console.log(`[Henrik] v3 API failed with mode=${mode}. Retrying without mode...`);
      const fallbackEndpoint = `/valorant/v3/by-puuid/matches/${r}/${puuid}?size=${size}`;
      rawList = await henrikFetch(fallbackEndpoint);
    } else {
      throw err;
    }
  }
  ...
  // Cache all matches in parallel, BUT DO NOT AWAIT so we can return to the user instantly
  const supabase = getServiceSupabase();
  const cachePromises = (rawList || []).map(async (match) => {
    if (!match.metadata?.matchid) return;
    ...
    const [matchRes, statsRes] = await Promise.all([matchPromise, statsPromise]);
    
    if (matchRes.error) console.error('[Supabase] matches v3 upsert error:', matchRes.error.message);
    if (statsRes.error) console.error('[Supabase] stats v3 upsert error:', statsRes.error.message);
  });
  
  // We fire and forget the cache updates so we don't block the API response
  Promise.all(cachePromises).catch(err => console.error('[Supabase] background cache error:', err));

  // Populate processedMatches synchronously
  ...
  // Ensure they remain sorted by game_start (descending) as returned by API
  processedMatches.sort((a, b) => (b.metadata?.game_start || 0) - (a.metadata?.game_start || 0));

  return processedMatches;
}

export async function henrikGetLifetimeMatches(name, tag, region = "na", page = 1, size = 10, mode = '') {
  const r = HENRIK_REGIONS[region] || "na";
  let endpoint = `/valorant/v1/lifetime/matches/${r}/${encodeURIComponent(name)}/${encodeURIComponent(tag)}?page=${page}&size=${size}`;
  if (mode && mode !== 'all') endpoint += `&mode=${mode}`;
  
  let response;
  try {
    response = await henrikFetch(endpoint);
  } catch (err) {
    if (mode && (err.status === 404 || err.status === 400)) {
      console.log(`[Henrik] v1 lifetime API failed with mode=${mode}. Retrying without mode...`);
      const fallbackEndpoint = `/valorant/v1/lifetime/matches/${r}/${encodeURIComponent(name)}/${encodeURIComponent(tag)}?page=${page}&size=${size}`;
      response = await henrikFetch(fallbackEndpoint);
    } else {
      throw err;
    }
  }
  ...
  // Cache full matches in the background so we don't block the API response
  // The UI will temporarily use the summary data, and future requests will be able to pull from cache.
  const cachePromises = summaries.map(async (summary) => {
    if (!summary.meta?.id) return;
    const matchId = summary.meta.id;
    try {
      await fetchAndCacheMatch(matchId, puuid);
    } catch (err) {
      console.error(`[Supabase] Background cache failed for match ${matchId}:`, err.message);
    }
  });
