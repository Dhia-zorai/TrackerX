...
async function fetchAndCacheMatch(matchId, puuid) {
  ...
  if (matchRes.error) console.error('[Supabase] matches upsert error:', matchRes.error.message);
  if (statsRes.error) console.error('[Supabase] player_match_stats upsert error:', statsRes.error.message);
