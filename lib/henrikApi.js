  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      if (res.status === 401 || res.status === 403) {
        const e = new Error("Henrik API key required or invalid. Add HENRIK_API_KEY to .env.local"); e.status = 401; throw e;
      }
      if (!res.ok) {
      ...
      console.log(`[Henrik] Data received: ${json.status}`);
...
// size max 10 per request; we call multiple times if needed
