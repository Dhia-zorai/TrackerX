  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      if (err.status !== 404 && err.status !== 401) {
        console.error(`[Henrik] Attempt ${attempt + 1} failed: ${err.message}`);
      }
...
export async function henrikGetMatchesByPuuid(puuid, region = "na", size = 10, mode = "") {
  ...
  const cachePromises = (rawList || []).map(async (match) => {
    ...
    if (matchRes.error || statsRes.error) {
      // Silent — background cache errors are non-critical
    }
  });
...
export async function henrikGetMMR(name, tag, region = "na") {
  ...
  if (data && data.puuid) {
    const supabase = getServiceSupabase();
    await supabase.from('mmr_snapshots').insert({
      puuid: data.puuid,
