...
function buildExportPayload(account, region, matches, puuid, aggregated, agentStats) {
  ...
  const matchRows = (matches || []).map(m => {
    ...
    if (advanced.spike_plants) totalSpikePlants += advanced.spike_plants;
    if (advanced.spike_defuses) totalSpikeDefuses += advanced.spike_defuses;
    if (advanced.trade_kills) totalTradeKills += advanced.trade_kills;
    if (advanced.traded_deaths) totalTradedDeaths += advanced.traded_deaths;
    if (advanced.clutch_attempts) totalClutchAttempts += advanced.clutch_attempts;
    if (advanced.clutch_wins) totalClutchWins += advanced.clutch_wins;
    if (advanced.eco_rounds) totalEcoRounds += advanced.eco_rounds;

    if (advanced.multi_kill_rounds) {
      Object.keys(advanced.multi_kill_rounds).forEach(k => {
        totalMultiKillRounds[k] = (totalMultiKillRounds[k] || 0) + advanced.multi_kill_rounds[k];
      });
    }

    if (advanced.weapons && Array.isArray(advanced.weapons)) {
      advanced.weapons.forEach(w => {
        if (!weaponMap[w.name]) {
          weaponMap[w.name] = { name: w.name, kills: 0, headshots: 0, bodyshots: 0, legshots: 0 };
        }
        weaponMap[w.name].kills += w.kills;
        weaponMap[w.name].headshots += w.headshots;
        weaponMap[w.name].bodyshots += w.bodyshots;
        weaponMap[w.name].legshots += w.legshots;
      });
    }
    ...
    return {
      ...
      spikePlants: advanced.spike_plants,
      spikeDefuses: advanced.spike_defuses,
      tradeKills: advanced.trade_kills,
      tradedDeaths: advanced.traded_deaths,
      clutchAttempts: advanced.clutch_attempts,
      clutchWins: advanced.clutch_wins,
      ecoRounds: advanced.eco_rounds,
      multiKillRounds: advanced.multi_kill_rounds,
  ...
  return {
    ...
    overallStats: {
      ...
      clutchSuccessRate: totalClutchAttempts > 0 ? parseFloat(((totalClutchWins / totalClutchAttempts) * 100).toFixed(1)) : null
    },
    agents,
    maps: mapStats,
    roundImpact: {
      openingDuelSuccessRate: totalFb !== null && totalFd !== null && (totalFb + totalFd) > 0 ? parseFloat(((totalFb / (totalFb + totalFd)) * 100).toFixed(1)) : null,
      firstBloodRate: matchesWithFbData > 0 ? parseFloat(((totalFb / (globalAtkRounds + globalDefRounds || matchesWithFbData)) * 100).toFixed(1)) : null,

      tradeKills: totalTradeKills,
      tradedDeaths: totalTradedDeaths,
      multiKillRounds: totalMultiKillRounds,
      ecoPerformance: totalEcoRounds, // Simplified tracking for eco rounds played
      antiEcoPerformance: null, // Hard to calculate definitively without full opponent econ
    },
    weapons: Object.values(weaponMap).sort((a, b) => b.kills - a.kills),
    teamImpact: {
      spikePlants: totalSpikePlants,
      spikeDefuses: totalSpikeDefuses,
