export function computeMatchStats(rawMatch, puuid) {
  ...
  let clutchWins = 0;
  let ecoRounds = 0;
  let multiKillRounds = { 2: 0, 3: 0, 4: 0, 5: 0 };
  let weapons = {};
  ...
  if (rounds.length > 0) {
    rounds.forEach((round, roundIndex) => {
      ...
      const plantPuuid = round.plant_events?.planted_by?.puuid || round.plant_events?.planted_by;
      const defusePuuid = round.defuse_events?.defused_by?.puuid || round.defuse_events?.defused_by;
      if (plantPuuid === puuid) spikePlants++;
      if (defusePuuid === puuid) spikeDefuses++;

      let k = false, a = false, s = true, t = false;
      let killEvents = round.kill_events;
      
      // Fallback for Henrik v2 API which places kills at the root level
      if (!killEvents && rawMatch.kills && Array.isArray(rawMatch.kills)) {
        // Handle 0-indexed or 1-indexed round numbering
        killEvents = rawMatch.kills.filter(kill => 
          kill.round === roundIndex || kill.round === (roundIndex + 1)
        );
        // De-duplicate if it matches both somehow
        killEvents = killEvents.filter((v, i, a) => a.findIndex(t => (t.kill_time_in_round === v.kill_time_in_round && t.killer === v.killer)) === i);
      }
      ...
      if (pStats) {
        const loadoutValue = pStats.economy?.loadout_value || 0;
        if (loadoutValue > 0 && loadoutValue < 2500) ecoRounds++;
      }
      ...
      killEvents.forEach(ke => {
        ...
        const killerTeam = ke.killer_team;
        const victimTeam = ke.victim_team;
        
        if (killerPuuid === puuid) {
          k = true;
          const weaponName = ke.damage_weapon_name || ke.weapon?.name || 'Unknown';
          if (!weapons[weaponName]) weapons[weaponName] = { name: weaponName, kills: 0, headshots: 0, bodyshots: 0, legshots: 0 };
          weapons[weaponName].kills++;
          
          // Check for trade kill
          const tradeKill = killEvents.some(k2 => {
            const k2Killer = k2.killer_puuid || k2.killer?.puuid || k2.killer;
            const k2VictimTeam = k2.victim_team;
            const timeDiff = (ke.kill_time_in_match || ke.kill_time_in_round || 0) - (k2.kill_time_in_match || k2.kill_time_in_round || 0);
            return k2Killer === victimPuuid && k2VictimTeam === teamId && timeDiff > 0 && timeDiff <= 5000;
          });
          if (tradeKill) tradeKills++;
        }
      ...
      // Clutch check: did all 4 teammates die before the round ended?
      const teammateDeaths = killEvents.filter(ke => {
        const vp = ke.victim_puuid || ke.victim?.puuid || ke.victim;
        return ke.victim_team === teamId && vp !== puuid;
      });
      if (teammateDeaths.length === 4) {
        // Find time of 4th teammate's death
        const lastTeammateDeathTime = Math.max(...teammateDeaths.map(ke => ke.kill_time_in_match || ke.kill_time_in_round || 0));
        // Check if player died AFTER the 4th teammate, or survived
        let playerDiedBefore = false;
        killEvents.forEach(ke => {
          const vp = ke.victim_puuid || ke.victim?.puuid || ke.victim;
          if (vp === puuid) {
            const myDeathTime = ke.kill_time_in_match || ke.kill_time_in_round || 0;
            if (myDeathTime <= lastTeammateDeathTime) playerDiedBefore = true;
          }
        });
        if (!playerDiedBefore) {
          clutchAttempts++;
          if (roundWon) clutchWins++;
        }
      }
      
      if (pStats && pStats.kills > 1) {
        const kValue = Math.min(pStats.kills, 5);
        if (multiKillRounds[kValue] !== undefined) multiKillRounds[kValue]++;
      } else if (!pStats && killEvents.length > 0) {
        // Fallback calculation for multi-kills if pStats is missing
        let roundKills = 0;
        killEvents.forEach(ke => {
          const killerPuuid = ke.killer_puuid || ke.killer?.puuid || ke.killer;
          if (killerPuuid === puuid) roundKills++;
        });
        if (roundKills > 1) {
          const kValue = Math.min(roundKills, 5);
          if (multiKillRounds[kValue] !== undefined) multiKillRounds[kValue]++;
        }
      }
  ...
  return {
    ...
    spike_plants: spikePlants,
    spike_defuses: spikeDefuses,
    trade_kills: tradeKills,
    traded_deaths: tradedDeaths,
    clutch_attempts: clutchAttempts,
    clutch_wins: clutchWins,
    eco_rounds: ecoRounds,
    multi_kill_rounds: multiKillRounds,
